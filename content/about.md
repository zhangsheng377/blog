...
* **主要项目**：
  ...
  4. 负责我司行业大模型的微调部分。作为开源蓝军路线的领导人，提出并尝试了模拟退火、dpo、细化二三级子领域能力分类、数据平权采样、基于裁判模型的预料质量评估、重复训练波动分析、数据精细化管理方案等有效实践，并积极分享，主力红军路线成功。
  
  5. 负责我司自动化评测系统的设计与开发。提出了从nginx的工程设计，到规则与大模型共存的评测方案，以及将rag引入评测系统，以此增强评测系统对于错误多样性的适应性。使用裁判模型作为rm，打通dpo训练。
