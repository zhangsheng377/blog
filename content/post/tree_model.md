...
但是，按照我们一般的感觉，好像如果简单地把一堆学习器相结合，它的结果应该是比最差的要好一点，但是比最好的要差一点。那么，我们应该怎样才能得到比最好的学习器还要好的结合效果呢？
...
而前面所说的，可以进行并行构建的bagging算法，它的主要思想其实是，对训练集进行抽样，从而用不同的子训练集去并行的生成各个基模型。
...
Schapire这个人在1995年，证明了 强可学习与弱可学习是等价的，这么一个概念。而所谓弱可学习就是，这个模型的正确率只比随机猜测要好一点点。

所以，一般大多数的方法是，去改变训练数据的概率或权重分布，从而针对不同的训练数据，训练出一系列的弱分类器。
...
因为提升树在具体实现时，采用了加法模型和前向分布算法来实现学习的优化过程，但是对于一般损失函数而言，往往并不是那么容易优化。

所以，Freidman就提出了梯度提升(gradient boosting)算法。它主要思想是去计算损失函数的负梯度，从而将其作为残差的近似值，来改变错误分类的权重。说白了就是用梯度信息去进行加权。

而在梯度提升算法Gradient Boosting中，最有代表性的就是GBDT(Gradient Boosting Descision Tree) 梯度提升决策树。



讲到这边大家其实可以发现哦，我今天所讲的每一页都是前一页的一个延申，都比之前只加入了一点点的新东西。比如这个GBDT，它其实里面包含了DT(Descision Tree)决策树、Boosting 提升方法，和 Gradient Boosting 梯度提升方法，都是我们之前所讲过的东西。



## xgboost

上一张我们说到了GBDT，而这边的XGBoost(Extreme Gradient Boosting)，它其实是GBDT的一种高效实现。
