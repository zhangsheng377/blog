...
但boosting方法只是一个思路，所以接下来我们来讲一下它的一个比较有名的实现。
...
因为提升树在具体实现时，采用了加法模型和前向分布算法来实现学习的优化过程，但是对于一般损失函数而言，往往并不是那么容易计算。
...
这个名字大家其实可以发现哦，我今天所讲的每一页都是前一页的一个延申，都比之前只加入了一点点的新东西。比如这个GBDT，它其实里面包含了DT(Descision Tree)决策树、Boosting 提升方法(也就是用一组弱分类器综合成一个强分类器)，和 Gradient Boosting 梯度提升方法(也就是用梯度信息对弱分类器进行加权)，这些都是我们之前所讲过的东西。
...
最后再说一点，其实对于同质数据的处理，比如像是分类模型，它就比较关注文章的语义信息，所以只要文章embedding就够了。那么我们现在所在研究的一个方向，其实就是用多任务、多输出的深度模型，比如底层我们采用bert做一个共享参数的embedding层，然后上面连接3个用于输出的子网络层，分别学习一、二、三级的分类信息。这种树状多任务模型结构的好处是，对于共享层而言，它可以同时看到并学习到一、二、三级分类的信息，这样就可以起到一个纠偏的作用。在我们实际的实验中，这种多任务模型与之前，每层训练一个单任务的模型，相比，它的指标是有很大提升的。所以这个也是未来的一个演进方向。
