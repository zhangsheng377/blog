...
就像是这张图，我们应该如何去挑选一个好西瓜呢？

1. 首先，我们可以看它的纹理。如果纹理模糊，则是坏瓜。但如果纹理只是稍显模糊呢，那我们还需要去感受一下它的触感，那些摸起来硬滑的的才是好瓜；而摸起来有点软粘的瓜就坏了。
2. 但其实，我们买瓜的时候，大多数瓜的纹理都是蛮清晰的，那么这个时候呢，我们就可以去看一看瓜蒂，如果瓜蒂蜷缩呢，就是好瓜，瓜蒂硬挺的呢，则是坏瓜。
3. 但面对那种稍微蜷一点的呢，那我们还要去看它的色泽，青绿或浅白的都是好瓜。但如果是乌黑的呢，那我们又要去看它的触感了，硬滑的是好瓜，软粘的是坏瓜。

所以，这张图，其实也就是决策树，我们可以怎么用哦。假设这时候我买了一个西瓜，它的纹理是清晰的，瓜蒂是硬挺的，那我们根据这颗决策树，立马就可以知道，这是一颗坏西瓜了。

看到这里，大家可能会觉得，决策树模型怎么这么简单呀，就是人类的正常思维嘛。的确，决策树模型的原理就是这么简单，其实我们上面这幅图就已经基本可以算作是一棵决策树了，但为什么这边说是基本可以算作呢？是因为其中的判断条件并没有被量化，比如，纹理清晰，那究竟什么样算是清晰？还有，这个色泽青绿，这个青绿究竟是个什么样的颜色呢？然后抛开这些量化问题，决策树的原理当中，其实还有几个问题我们需要上升到理论的高度去解决：
...
那么，熵既然表示的是无序的程度，它就肯定是和变量的分布有关系的。于是，我们假设某个特征$X$，当它为$x_i$时的概率，是$p_i$：
...
那么，数据集$D$的熵，我们就可以定义为：
$$
H(D)=-\sum_{k=1}^K {{|C_k|}\over {|D|}} log_2 {{|C_k|}\over {|D|}}
$$
在这里，我们假设数据集$D$有K个类别，$|C_k|$表示类别$k$的样本个数，而底下的分母$|D|$表示样本总数，所以类别$k$的概率就是${|C_k|}\over {|D|}$了。
...
而我们刚才说的信息增益，其实有个小问题，就是它会偏向于去选择种类较多的那个特征。比如，编号 这个特征，它里面所有的取值都是唯一的，那么一旦选择这个特征进行划分，也就相当于是说：你告诉我编号，我立马就能知道它的最终分类。但其实在预测时，所见到的编号都是新的，也就是说编号这个特征在实际预测时，是没有用处的。所以，C4.5算法就使用信息增益比，来校正这个问题。

$$
g_R(D,A) = {g(D,A)\over H_A(D)}
$$

即，信息增益比$g_R(D,A)$，为其信息增益$g(D,A)$与数据集D关于特征A的熵$H_A(D)$ 之比。这样，也就相当于，是在信息增益的基础之上乘以了一个惩罚参数，当特征个数较多时，惩罚参数较小；特征个数较少时，惩罚参数较大。
...
这个就是基尼指数公式，其中的$p_k$就是样本属于$k$这个类别的概率，那么$(1-p_k)$就是这个样本被分错的概率。所以，基尼指数其实就等于$样本被选中的概率 * 样本被分错的概率$，那么，当我们选择基尼指数来进行特征划分的时候，其实就是在使得，划分之后的，子特征空间里的样本，被分错的概率在逐步地减小。
$$
...
但是，光有一个决策树模型去解决问题，可能还不够好。所以，我们就需要一种可以去集合多个模型来共同解决问题的方法，这个就是集成学习。
...
而它的理论基础呢，是Schapire这个人在1995年，证明了 强可学习与弱可学习是等价的，这么一个概念。所谓弱可学习就是，这个模型的正确率只比随机猜测要好一点点。而强可学习呢，就是指这个模型的效果会非常的好。而这个人证明了，强可学习和弱可学习，是等价的，意思就是说，不管所采用的方法是难还是易，总之我们一定可以找到一种方法，去把弱学习器给变成强学习器。

![img](https://pic1.zhimg.com/80/v2-6dfc944cd6c241d5fb47405173d957fc_720w.jpg)

但是，按照我们一般的感觉，好像会认为，如果简单地把一堆学习器相结合，也就是相加求平均，那么它的效果应该是比最差的要好一点，但是比最好的要差一点。
